...
export function renderRiderCards(riders) {
  ...
  el.innerHTML = riders.map(r => `
    <div class="list">
      <div class="list-row">
        <div class="userbox">
          <div class="avatar">${r.vehicleType === "car" ? "🚗" : "🏍️"}</div>
          <div>
            ${nameWithBadge(r.fullName || "Umumotari", r)}
            <div><small>${r.vehicleType === "car" ? "Imodoka" : "Moto"} · ${escapeHtml(r.plate || "")}</small></div>
...
export function renderTripCards(trips) {
  ...
  el.innerHTML = trips.map(t => `
    <div class="list">
      <div class="list-row">
        <div>
          <b>${t.vehicleType === "car" ? "🚗" : "🏍️"} ${escapeHtml(t.from || "—")} → ${escapeHtml(t.to || "—")}</b>
          ${typeof t.distanceKm === "number" ? `<div><small>${t.distanceKm} km${t.riderName ? " · " + escapeHtml(t.riderName) : ""}</small></div>` : ""}
        ...
        <div style="text-align:right">
          ...
          ${typeof t.fare === "number" ? `<div class="price" style="font-size:15px;margin-top:4px">${formatRWF(t.fare)}</div>` : ""}
          ${typeof t.fare === "number" && t.status !== "cancelled" && !t.paid ? `
            <button class="secondary" style="width:auto;padding:6px 12px;font-size:13px;margin-top:6px" onclick="window.payForTrip('${t.id}', ${t.fare})">💰 Ishyura</button>
          ` : ""}
          ${t.status === "completed" && !t.rated ? `
            <button class="secondary" style="width:auto;padding:6px 12px;font-size:13px;margin-top:6px" onclick="window.openRateTrip('${t.id}','${escapeHtml(t.riderName || "")}')">⭐ Tanga amanota</button>
          ` : ""}
        </div>
      </div>
    </div>
...
export function renderMarketCards(listings) {
  ...
  el.innerHTML = listings.map(l => `
    <div class="list">
      <div class="list-row">
        ${l.photoUrl ? `<img src="${l.photoUrl}" style="width:56px;height:56px;border-radius:11px;object-fit:cover;margin-right:10px">` : ""}
        <div>
...
export function renderMotorcycleCards(motorcycles) {
  ...
  el.innerHTML = motorcycles.map(m => `
    <div class="list">
      <div class="list-row">
        ${m.photoUrl ? `<img src="${m.photoUrl}" style="width:56px;height:56px;border-radius:11px;object-fit:cover;margin-right:10px">` : ""}
        <div>
...
export function renderBusinessCards(businesses) {
  ...
  el.innerHTML = businesses.map(b => `
    <div class="list">
      <div class="list-row">
        ${b.photoUrl ? `<img src="${b.photoUrl}" style="width:56px;height:56px;border-radius:11px;object-fit:cover;margin-right:10px">` : ""}
        <div>
...
export function renderProfile(user) {
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      ${user.role === "driver" ? `<button class="secondary" onclick="window.openPage('driverMode')">🏍️ Fungura Driver Mode</button>` : `<button class="secondary" onclick="window.registerAsDriver()">+ Iyandikishe nk'Umumotari/Dereva</button>`}
