...
/* =========================================================
   RATINGS — nyuma y'urugendo rurangiye
   ========================================================= */

window.openRateTrip = function (tripId, riderName) {
  const modal = document.getElementById("modal");
  const content = document.getElementById("modalContent");
  content.innerHTML = `
    <h2>⭐ Tanga amanota — ${escapeHtml(riderName || "umumotari")}</h2>
    <div id="starPicker" style="font-size:34px;text-align:center;margin:14px 0;cursor:pointer">☆☆☆☆☆</div>
    <label>Icyo wagize icyo uvuga (si ngombwa)</label>
    <input id="rateComment" placeholder="Urugero: Yari afite umuco mwiza">
    <button class="primary" id="submitRatingBtn">✅ Ohereza</button>
  `;
  modal.classList.remove("hidden");

  let selectedStars = 0;
  const starEl = document.getElementById("starPicker");
  starEl.onclick = e => {
    const rect = starEl.getBoundingClientRect();
    const clickX = e.clientX - rect.left;
    selectedStars = Math.max(1, Math.min(5, Math.ceil((clickX / rect.width) * 5)));
    starEl.textContent = "★".repeat(selectedStars) + "☆".repeat(5 - selectedStars);
  };

  document.getElementById("submitRatingBtn").onclick = async () => {
    if (selectedStars === 0) { alert("Hitamo inyenyeri nibura imwe."); return; }
    const comment = document.getElementById("rateComment").value.trim();

    await DataStore.add("ratings", {
      tripId, passengerUid: window.App.currentUser.uid,
      stars: selectedStars, comment
    });
    await DataStore.setDoc("trips", tripId, { rated: true });
    window.closeModal();
    renderTrips();
  };
};
