...
export async function renderAdmin() {
  ...
  container.innerHTML += await renderAuditLogCard();
...
async function renderAuditLogCard() {
  const logs = (await DataStore.list("adminLogs")).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)).slice(0, 20);
  return `
    <div class="card">
      <h3>📜 Audit Log (ibikorwa 20 bwa nyuma)</h3>
      ${logs.length === 0 ? emptyState("Nta bikorwa byanditswe kuri ubu.") : logs.map(l => `
        <div class="list">
          <b>${escapeHtml(l.action)}</b> — ${escapeHtml(l.targetId || "")}
          <div><small>${new Date(l.timestamp).toLocaleString("rw-RW")}</small></div>
        </div>
      `).join("")}
    </div>
  `;
...
/* =========================================================
   AUDIT LOG — amateka y'ibikorwa by'ingenzi bya Owner/Admin.
   Ntabwo bisibwa cyangwa bihindurwa (reba firestore.rules).
   ========================================================= */

async function writeAuditLog(action, targetId, oldValue, newValue, reason = "") {
  try {
    await DataStore.add("adminLogs", {
      actorId: window.App.currentUser?.uid || "unknown",
      action, targetId,
...
window.saveFareSettings = async function () {
  ...
  try {
    const oldTiers = await getFareTiers();
    const newTiers = {
      moto: {
    ...
    await writeAuditLog("fare_changed", "fareConfig",
      { moto: oldTiers.moto, car: oldTiers.car }, newTiers);
...
window.toggleVerify = async function (collectionName, id, nextValue) {
  ...
  await writeAuditLog(nextValue ? "verified" : "unverified", `${collectionName}/${id}`, !nextValue, nextValue);
  await renderAdmin();
